...
from knowledge_core.kb import KnowledgeCore, DEFAULT_MODEL

from .generators import generate_flat_triples, generate_typed_triples, generate_hierarchy
...
class TestBenchMaterialise:
    ...
    @pytest.mark.parametrize("n", MATERIALISE_SIZES, ids=[f"{n}" for n in MATERIALISE_SIZES])
    ...
    def test_with_reasoner_typed(self, benchmark, n):
        """materialise() with reasoner on typed triples (classes + properties)."""
        kb = KnowledgeCore(enable_reasoner=True)
...
class TestBenchMaterialiseHierarchy:
    ...
    @pytest.mark.skipif(not HAS_REASONER, reason="reasonable not installed")
    @pytest.mark.parametrize("depth", HIERARCHY_DEPTHS, ids=[f"depth_{d}" for d in HIERARCHY_DEPTHS])
...
class TestBenchReasonerSteps:
    ...
    def test_graph_copy_back(self, benchmark, n):
        """Time: Graph() += r.reason() -- copying inferred triples into new graph."""
        kb = KnowledgeCore(enable_reasoner=True)
        kb.update(generate_flat_triples(n))
        graph = kb.models[DEFAULT_MODEL].graph
